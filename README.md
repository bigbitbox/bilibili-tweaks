...
5. 如果使用 **Vimium C**，在 B 站的排除规则中将 `1 2 3 4` 加入放行键，并保留已有放行键。可用匹配规则 `:https://www.bilibili.com/`。否则数字键可能被 Vimium C 当作重复次数拦截，倍速提示也不会出现。保存后若当前页仍无反应，先按一次 Esc 清除未完成的按键序列，再在非输入框区域重试。
